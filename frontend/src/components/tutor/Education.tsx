...
const Education = ({ onSaveSuccess }: EducationProps) => {
  ...
  const onSubmit = async (data: EducationForm) => {
    try {
      ...
      const submitData = {
        ...data,
        startDate: convertDateToISO(data.startDate), // Ensure endDate is undefined if isOngoing is true
        endDate:
          data.endDate && !data.isOngoing
      ...
      if (editingId) {
        const response = await api.put(
          `/tutor/profile/education/${editingId}`,
          submitData
        );
  ...
  const handleEdit = (edu: EducationItem) => {
    ...
    setValue("location", edu.location); // Convert to date input format (YYYY-MM-DD) for the calendar
    setValue("startDate", formatDateStringToDateInput(edu.startDate));
    setValue(
      "endDate",
      edu.endDate ? formatDateStringToDateInput(edu.endDate) : ""
    );
        ...
        if (response.data?.profileCompletion !== undefined) {
          setFeedback(
            response.data.profileCompletion === 100
              ? "Education removed • Profile remains complete"
              : "Education removed"
          );
          setTimeout(() => setFeedback(""), 3000);
        }
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
                   {" "}
          {errors.degreeTitle && (
            <p className="error-text">{errors.degreeTitle.message}</p>
          )}
                 {" "}
        </div>
               {" "}
        <div>
          ...
                   {" "}
          {errors.university && (
            <p className="error-text">{errors.university.message}</p>
          )}
                 {" "}
        </div>
               {" "}
          ...
          <div>
            ...
            {watch("startDate") && (
              <p className="text-sm text-gray-600 mt-1">
                                Selected:{" "}
                {formatDateToMMDDYYYY(watch("startDate"))}             {" "}
              </p>
            )}
                       {" "}
            {errors.startDate && (
          ...
          educations.map((edu) => (
            <div key={edu.id} className="border border-gray-200 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-600">
                                {formatDateToMMDDYYYY(edu.startDate)} -{" "}
                {edu.isOngoing
                  ? "Ongoing"
                  : edu.endDate
                  ? formatDateToMMDDYYYY(edu.endDate)
                  : "N/A"}
                             {" "}
              </p>
                         {" "}
