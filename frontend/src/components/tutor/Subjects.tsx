...
                const tutorSubjectToRemove = tutorSubjects.find(ts => ts.subjectId === subjectId);
                if (tutorSubjectToRemove) {
                  await api.delete(`/tutor/profile/subjects/${tutorSubjectToRemove.id}`)
                }
          }
...
      setErrorMessage('Error updating subjects. Please check your selections and try again.')
    } finally {
      setLoading(false)
    }
  }
...
          {!selectedCategory ? (
            <div>
              <h3 className="text-lg font-semibold mb-4">Step 1: Select a Category</h3>
...
                  Step 2: Select Subjects in {categories.find(c => c.id === selectedCategory)?.name}
                </h3>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
...
                  <p className="font-medium mb-2">No subjects listed under this category yet.</p>
                </div>
              )}
            </div>
          )}
...
        <div className="mt-8">
          <h3 className="text-lg font-semibold mb-3">✅ Your Current Teaching Subjects</h3>
