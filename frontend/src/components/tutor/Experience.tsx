...
// Define the shape of the component's expected props
interface ExperienceProps {
  onSaveSuccess: () => void; // Function to call on successful save
}

// 1. UPDATED: Add teachingMode to the Experience Item Interface
...
// 2. UPDATED: Add teachingMode to the Form Data Interface
...
        // If isCurrent is true, ensure endDate is undefined for backend to handle as null
        endDate: data.endDate && !data.isCurrent ? convertDateToISO(data.endDate) : undefined
      }
      
      if (editingId) {
...
      // Optionally display a more detailed error message to the user
      setFeedback('Error: Could not save experience. Check the form fields.')
    } finally {
      setLoading(false)
    }
  }
...
    setValue('teachingMode', exp.teachingMode) // Load existing teaching mode
...
        </div>
        
        {/* ADDED: Teaching Mode Input Field */}
        <div>
...
            {/* Ensure these match your Prisma TeachingMode ENUM values */}
...
                    Mode: {exp.teachingMode.replace('_', ' ')}
                  </p>
                </div>
                <div className="flex gap-2">
