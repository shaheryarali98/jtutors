...
      setBackgroundCheckStatus('PENDING')
      // Update feedback message to be generic for both submission and update
...
      
      // 2. REMOVED the 'if (isNewSubmission)' check.
      // The redirection now runs unconditionally after a successful save.
      setTimeout(() => {
        // Primary redirect logic using React Router's navigate
        navigate('/tutor/dashboard'); 
        // Call the prop too, for external workflow compatibility (if used)
        if (onSaveSuccess) {
          onSaveSuccess();
        }
